  ...
  const run = async () => {
    while (true) {
      ...
      try {
        ...
        // log out
        console.log(`${tasks[taskIndex]} defintion: ${JSON.stringify(result)}`);
...
export async function generateDefsFast(words) {
  const startTime = Date.now();

  let definitions;

  try {
    definitions = await processWithConcurrencyLimit(
      words,
      CONCURRENCY_LIMIT,
      generateDefinition
    );
    ...
    console.error(
      `Fatal error generating definition for "${words
        .map((w) => w)
        .join(", ")}":`,
      err.message
    );
